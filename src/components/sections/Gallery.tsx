...
import { gallery } from '../../data/scrapbook'
import { SectionHeading } from '../ui/SectionHeading'

export function Gallery({ onHeart }: { onHeart: () => void }) {
  const [selected, setSelected] = useState<number | null>(null)
  const move = (amount: number) => setSelected(value => value === null ? null : (value + amount + gallery.length) % gallery.length)
  useEffect(() => {
  ...
  return (
    <section id="gallery" className="section relative overflow-hidden">
      ...
      <div className="mx-auto grid max-w-6xl grid-cols-2 gap-4 md:grid-cols-3 md:gap-9">
        {gallery.map((photo, i) => <motion.button initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * .07 }} whileHover={{ y: -10, rotate: 0, scale: 1.02 }} onClick={() => setSelected(i)} key={photo.src} style={{ rotate: `${[-3, 2, -1, 3, -2, 1][i]}deg` }} className="polaroid"><img loading="lazy" src={photo.src} alt={photo.alt} /><span>{photo.caption}</span></motion.button>)}
      </div>
      ...
      <AnimatePresence>{selected !== null && <motion.div role="dialog" aria-modal="true" aria-label="Photo lightbox" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[80] grid place-items-center bg-[#151211]/95 p-5 backdrop-blur-xl">
        ...
        <motion.figure key={selected} initial={{ opacity: 0, scale: .96 }} animate={{ opacity: 1, scale: 1 }} className="max-w-4xl text-center"><img src={gallery[selected].src} alt={gallery[selected].alt} className="max-h-[72vh] rounded-lg object-contain shadow-2xl" /><figcaption className="mt-5 font-script text-xl italic text-stone-200">{gallery[selected].caption}</figcaption></motion.figure>
